...
export const Footer = (props: FooterProps) => {
  ...
  const handleSubscribe = async () => {
    try {
      ...
      console.error(
        `Could not subscribe: ${e.message ? e.message : e.toString()}`
      );
      toast.error(
        `Something went wrong, we could add you to our subscriber list`,
        { icon: "😞️" }
      );
    }
  };
  ...
  return (
    <div className="grid gap-4 p-10 text-center text-white md:justify-items-center md:grid-cols-10 md:text-left md:p-20 font-karla bg-primary">
      {/* Subscribe */}
      <div className={"md:col-span-4 xl:col-span-3 grid gap-4 mb-4"}>
        <p className={"text-lg "}>Sign up for our monthly newsletter</p>
      ...
      <ul className="md:col-span-2">
        <li>
          <Link href={"/about"}>
            <a className="duration-300 hover:text-secondary">About</a>
          </Link>
        </li>
        <li>
          <Link href={"/contact"}>
            <a className="duration-300 hover:text-secondary whitespace-nowrap">
              Contact Us
            </a>
          </Link>
        </li>
      ...
      <div className="font-medium tracking-wide text-white md:col-span-2">
        ...
        <div
          className={"justify-center flex  flex-nowrap gap-4 hover:bg-primary"}>
          {/* FIXME: Facebook */}
          <SocialIcon
            href={"https://www.facebook.com/ThePerfectHealthPractice/"}>
            <svg
              viewBox="0 0 24 24"
              fill="currentColor"
          ...
          <SocialIcon href={"https://www.instagram.com/perfecthealthpractice/"}>
            <svg
              viewBox="0 0 30 30"
              fill="currentColor"
              className="h-6 hover:text-secondary">
